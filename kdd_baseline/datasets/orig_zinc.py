...
from datasets.utils.data import from_smiles
...
class ZINC(InMemoryDataset):
    ...
    def download(self):
        ...
        os.rename(
            osp.join(self.root, self.name, "250k_rndm_zinc_drugs_clean_3.csv"),
            self.raw_paths[0],
        )
            ...
            for i, raw_data in tqdm(enumerate(csv_reader)):
                ...
                for cycle in cycles:
                    if cycle.__len__() >= 6:
                        num_cycles_le6 += 1
                data.y = (
                    float(raw_data[fields["logP"]])
                    - float(raw_data[fields["SAS"]])
                    - float(num_cycles_le6)
                )
