...
from datasets.orig_zinc import ZINC
    ...
    def get_scaffold(self, smile: str) -> str:
        ...
        try:
            scaffold = MurckoScaffold.MurckoScaffoldSmiles(
                mol=Chem.MolFromSmiles(smile), includeChirality=False
            )
            return scaffold
...
class GOODZINC(InMemoryDataset):
    ...
    def download(self):
        path = gdown.download(
            self.url, output=osp.join(self.raw_dir, self.name + ".zip"), fuzzy=True
        )
    ...
    def get_no_shift_list(self, data_list):
        ...
        train_list, val_list, test_list = (
            ...
            data_list[train_split:val_split],
            data_list[val_split:],
        )
    ...
    def get_covariate_shift_list(self, sorted_data_list):
        ...
        for i, data in enumerate(sorted_data_list):
            if (
                cur_env_id < 2
                and i >= train_val_test_split[cur_env_id + 1]
                and data.domain_id != cur_domain_id
            ):
        ...
        for i, data in enumerate(train_list):
            if (
                cur_env_id < 9
                and i >= (cur_env_id + 1) * num_per_env
                and data.domain_id != cur_domain_id
            ):
                # if i >= (cur_env_id + 1) * num_per_env:
                cur_env_id += 1
        ...
        train_list, id_val_list, id_test_list = (
            train_list[: -2 * num_id_test],
    ...
    def get_concept_shift_list(self, sorted_domain_split_data_list):
        ...
        is_val_split = [
            False if i < len(is_train_split) - 1 else True
            for i in range(len(is_train_split))
        ]
        is_test_split = [
            not (tr_sp or val_sp) for tr_sp, val_sp in zip(is_train_split, is_val_split)
        ]
        ...
        while cur_split < len(env_list):
            ...
            for domain_id, each_domain_datas in enumerate(
                ...
                for i in range(len(each_domain_datas) - 1):
                    ...
                    if cur_split == len(env_list) - 1:
                        ...
                        if oc * (data.pyx - global_mid_pyx) > 0:
                            ...
                            if (
                                data_rand < bias_connect[cur_split]
                                and picking_rand < split_picking_ratio[cur_split]
                            ):
                ...
                if (
                    both_label_domain[0]
                    and both_label_domain[1]
                    and (both_label_include[0] or both_label_include[1])
                ):
                    extra_data = None
            ...
            cur_split += 1
            num_train = sum(
                [len(env) for i, env in enumerate(env_list) if is_train_split[i]]
            )
            num_val = sum(
                [len(env) for i, env in enumerate(env_list) if is_val_split[i]]
            )
            num_test = sum(
                [len(env) for i, env in enumerate(env_list) if is_test_split[i]]
            )
        ...
        train_list, ood_val_list, ood_test_list = (
            list(
                itertools.chain(
                    *[env for i, env in enumerate(env_list) if is_train_split[i]]
                )
            ),
            list(
                itertools.chain(
    ...
    def process(self):
        ...
        sorted_data_list, sorted_domain_split_data_list = self.get_domain_sorted_list(
            data_list, domain=self.domain
        )
        covariate_shift_list = self.get_covariate_shift_list(deepcopy(sorted_data_list))
        print()
        print("#IN#Covariate shift dataset done!")
        concept_shift_list = self.get_concept_shift_list(
            deepcopy(sorted_domain_split_data_list)
        )
        print()
    ...
    @staticmethod
    def load(
        dataset_root: str, domain: str, shift: str = "no_shift", generate: bool = False
    ):
        r"""
        ...
        train_dataset._data.y = train_dataset._data.y.reshape(-1, 1)
        if id_val_dataset:
            id_val_dataset._data.y = id_val_dataset._data.y.reshape(-1, 1)
            id_test_dataset._data.y = id_test_dataset._data.y.reshape(-1, 1)
        val_dataset._data.y = val_dataset._data.y.reshape(-1, 1)
        test_dataset._data.y = test_dataset._data.y.reshape(-1, 1)
